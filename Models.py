...
class Plant():
    ...
    def forward(self,t,x,u):
        x_ = x[t-1] + self.cop/(self.m*self.cp) * u[t-1] 
        return x_
...
class PsiX(nn.Module):  #
    def __init__(self, Plant):
        super().__init__()
        plant = Plant(m=100,cp=4186,cop = 2)
        self.f = plant.forward
...
class Controller(nn.Module):
    ...
    def forward(self, t, x, u,xi):
        w = x[t] - self.psi_x(t, x, u)  # Reconstructing noise

        u, xi_ = self.psi_u(torch.tensor([w]), torch.tensor(xi),t)  # Feeding the noise into the controller and computing u and updated
        # controller state
        return u, xi_
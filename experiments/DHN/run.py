...
loss_fn = DHNLoss(
    R=10*args.alpha_u, u_min=u_min, u_max=u_max, x_min=x_min,x_max=x_max,
    alpha_xh=20,    
    alpha_xl=30,
)
...
for epoch in range(1+args.epochs):
    ...
    if epoch%args.log_epoch == 0:
        msg = 'Epoch: %i --- train loss: %.2f --- Loss xl : %.2f ---  loss u min: %.2f'% (epoch, loss, 0,0)
        ...
        if args.return_best:
            ...
            msg += ' ---||--- validation loss: %.2f  --- Loss x low: %.2f ---  loss u min: %.2f ---  loss u low: %.2f---  loss u high: %.2f' % (
                loss_valid,0,0,0,0)
